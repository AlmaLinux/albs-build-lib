        ...
        for key, value in sorted(options.items()):
            if key == 'repositoryid':
                ...
                if value not in ('roundrobin', 'priority'):
                    raise ValueError(
                        'unsupported failovermethod value {0}'.format(value)
                    )
...
class YumConfig(BaseYumConfig):
    ...
    def __init__(
        ...
        """
        if rpmverbosity is not None and rpmverbosity not in (
            'info',
            'critical',
            'emergency',
            'error',
            ...
            raise ValueError(
                'invalid rpmverbosity value "{0}"'.format(rpmverbosity)
            )
        self.__data = {
            k: v
            for (k, v) in iter(list(locals().items()))
    ...
    def add_repository(self, repo):
        ...
        if not isinstance(repo, YumRepositoryConfig):
            raise ValueError(
                'repository type {0} is not supported'.format(type(repo))
            )
        if repo.name in self.__repos:
            raise ValueError(
                'repository {0} is already added'.format(repo.name)
            )
...
# TODO: Make sure that there is no conflicts
class YumRepositoryConfig(BaseYumConfig):
    ...
    def render_config(self):
        ...
        """
        section = self.trim_non_empty_string(
            'repositoryid', self.__data['repositoryid']
        )
