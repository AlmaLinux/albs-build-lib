...
from albs_common_lib.utils.proc_utils import (
...
class MockSupervisor():
    ...
    def __init__(
        ...
        self.__root_dir = root_dir or '/var/lib/mock'
        self.__cache_dir = cache_dir or '/var/cache/mock'
        self.__sys_dirs_to_umount = sys_dirs_to_umount
    ...
    def environment(self, mock_config):
        ...
        with self.__db.begin(write=True) as txn:
            ...
            config_file = next(
                (
                    c
                    for c in existent_configs
                    if not txn.get(c.encode('utf-8'), db=locks_db)
                ),
                None,
            )
            if not config_file:
    ...
    def free_environment(self, environment):
        ...
        try:
            ...
            self.__log.error(
                '{0} mock environment cleanup failed with {1} '
                'exit code: {2}'.format(config_file, e.exit_code, e.stderr)
            )
        with self.__db.begin(write=True) as txn:
            locks_db = self.__db.open_db(b'locks', txn=txn)
            txn.put(
                config_file.encode('utf-8'), b'', dupdata=False, db=locks_db
            )
        ...
        for config_file, lock_data in locks_cursor.iternext():
            if lock_data:
                ...
                if is_pid_exists(pid):
                    ...
                    continue
                self.__log.warning(
                    '{0} mock environment is locked by dead '
                    'process {1}, marking it as free'.format(config_file, pid)
                )
                locks_cursor.put(config_file, b'', dupdata=False)
            ...
            config_path = os.path.join(
                self.__storage_dir.encode('utf-8'), config_file
            )
            # For some reason the file was not removed when deleting
            # the environment, so cleaning it now
            ...
            if current_ts - usage_ts > self.__idle_time:
                ...
                self.__log.info(
                    'deleting expired environment {0}'.format(config_file)
                )
                ...
                self.__log.info(
                    'updating outdated environment {0}'.format(config_file)
                )
                self.__scrub_mock_environment(config_path.decode('utf-8'))
                txn.put(
                    config_file,
                    struct.pack('iii', current_ts, current_ts, 0),
                    db=stats_db,
                )
    ...
    def __init_storage(self):
        ...
        if not os.path.exists(self.__storage_dir):
            self.__log.info(
                'initializing mock supervisor storage in the {0} '
        ...
        if not os.path.exists(dst):
            os.symlink(src, dst)
        return lmdb.open(
            os.path.join(self.__storage_dir, 'mock_supervisor.lmdb'), max_dbs=2
        )
    ...
    def __find_existent_configs(self, config_hash):
        ...
        return [
            c
            for c in os.listdir(self.__storage_dir)
    ...
    def __update_usage_count(self, txn, config_file):
        ...
        txn.put(
            key,
            struct.pack('iii', creation_ts, usage_ts, usages + 1),
            db=stats_db,
        )
    ...
    def __update_usage_timestamp(self, txn, stats_db, config_file):
        ...
        txn.put(
            key,
            struct.pack('iii', creation_ts, round(time.time()), usages),
            db=stats_db,
        )
    ...
    def __raise_missing_stats_error(self, config_file):
        ...
        MockSupervisorError
        """
        err = 'there is no statistics found for {0} mock environment'.format(
            config_file
        )
        self.__log.error(
            '{0}. Please verify the database integrity'.format(err)
        )
    ...
    def __scrub_mock_environment(self, config_path):
        ...
        counter = 5
        last_exc = None
        while counter > 0 and not success:
            try:
                mock_env.scrub('all')
            except MockError as e:
                last_exc = e
