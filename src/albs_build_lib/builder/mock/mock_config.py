...
def to_mock_config_string(value):
    ...
    raise ValueError(
        'unsupported type {0} of "{1}" value'.format(type(value), value)
    )
...
class MockConfig():
    ...
    def add_module_install(self, module_name: str):
        """
    ...
    def add_module_enable(self, module_name: str):
        """
    ...
    def add_file(self, chroot_file):
        ...
        if chroot_file.name in self.__files:
            raise ValueError(
                'file {0} is already added'.format(chroot_file.name)
            )
    ...
    def add_plugin(self, plugin):
        ...
        if plugin.name in self.__plugins:
            raise ValueError(
                'plugin {0} is already configured'.format(plugin.name)
            )
        self.__plugins[plugin.name] = plugin

    @staticmethod
    def get_default_legal_host_arches(target_arch: str):
        """
        ...
        # TODO: Investigate if 32-bit packages will really be able to be built on 64-bit ARM
            ...
            return ('riscv64',)
        raise ValueError(
            'there is no default_host_arches value for {0} '
        ...
        if isinstance(value, dict):
            for k, v in sorted(value.items()):
                out += 'config_opts[{0}][{1}] = {2}\n'.format(
                    option, to_mock_config_string(k), to_mock_config_string(v)
                )
        elif append:
            out += 'config_opts[{0}].append({1})\n'.format(
                option, to_mock_config_string(value)
            )
        else:
            out += 'config_opts[{0}] = {1}\n'.format(
                option, to_mock_config_string(value)
    ...
    def dump_to_file(self, config_file, root=None):
        ...
        try:
            ...
            for option, value_list in sorted(
                self.__append_config_opts.items()
            ):
                for value in value_list:
                    fd.write(
                        self.render_config_option(option, value, append=True)
                    )
    ...
    def config_hash(self):
        ...
        if self.__config_opts.get('root'):
            logging.warning(
                'mock chroot "root" option is defined which is '
    ...
    def _add_module(self, option, module_name):
        ...
        if module_name in self.__config_opts[option]:
            raise ValueError(
                '{0} is already added to the {1}'.format(module_name, option)
            )
...
class MockPluginConfig():
    ...
    def render_config(self):
        ...
        out = 'config_opts["plugin_conf"]["{0}_enable"] = {1}\n'.format(
            self.__name, to_mock_config_string(self.__enable)
        )
        ...
        for key, opt in sorted(self.__opts.items()):
            out += (
                'config_opts["plugin_conf"]["{0}_opts"][{1}] = {2}\n'.format(
                    self.__name,
                    to_mock_config_string(key),
...
class MockPluginChrootScanConfig():
    """
    mock plugin configuration.
    """
    ...
    def render_config(self):
        ...
        for key, opt in sorted(self.__opts.items()):
            opts_dict[key] = opt
        out += (
            f'config_opts["plugin_conf"]["{self.__name}_opts"] = {opts_dict}\n'
        )
        return out
...
class MockBindMountPluginConfig(MockPluginConfig):
    ...
    def __init__(self, enable, mounts):
        ...
        """
        super().__init__('bind_mount', enable)
    ...
    def render_config(self):
        ...
        out = 'config_opts["plugin_conf"]["bind_mount_enable"] = {0}\n'.format(
            to_mock_config_string(self.enable)
        )
