...
def init_args_parser():
    ...
    list_parser = subparsers.add_parser(
        'list', description=list_help, help=list_help
    )
...
def format_unix_time(unix_ts):
    ...
    str
    """
    return datetime.datetime.fromtimestamp(unix_ts).strftime(
        '%Y-%m-%dT%H:%M:%S'
    )
...
def list_environments(db):
    with db.begin() as txn:
        ...
        for config_file, lock_data in locks_cursor.iternext():
            if lock_data:
                status = 'locked by {0} process "{1}" thread'.format(
                    *struct.unpack('i20p', lock_data)
                )
            else:
                status = 'free'
            ...
            if stats_data:
                ...
                stats = 'created {0}, used {1}, {2} times in total'.format(
                    format_unix_time(creation_ts),
                    format_unix_time(usage_ts),
                    usages,
                )
            else:
...
def main(sys_args):
    ...
    if not os.path.exists(os.path.join(args.storage, 'mock_supervisor.lmdb')):
        print(
            '{0} is not a mock environments storage '
            'directory'.format(args.storage),
            file=sys.stderr,
        )
        return 1
    db = lmdb.open(
        os.path.join(args.storage, 'mock_supervisor.lmdb'), max_dbs=2
    )
