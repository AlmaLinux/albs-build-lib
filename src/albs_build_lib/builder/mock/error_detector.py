...
class MockErrorEnum(enum.IntEnum):
    MOCK_ERR_SPEC_SECTION = 0  # wrong exit status from an RPM file section
    MOCK_ERR_UNMET_DEPENDENCY = 1  # unmet dependency (root log)
    MOCK_ERR_ARCH_EXCLUDED = 2  # excluded architecture
    MOCK_ERR_BUILD_HANGUP = 3  # build hangup (mostly relevant to PHP)
    MOCK_ERR_NO_FREE_SPACE = 4  # insufficient space in download directory
    MOCK_ERR_MISSING_FILE = 5  # missing file error (build log)
    MOCK_ERR_CHANGELOG_ORDER = 6  # wrong changelog records order (build log)
    MOCK_ERR_UNPACKAGED = 7  # installed but unpackaged files found
    MOCK_ERR_REPO = 8  # repository metadata or network error (root log)
    MOCK_ERR_TIMEOUT = 9  # build timeout error


MockErrorRecap = collections.namedtuple(
    'MockErrorRecap',
    ['error_code', 'error_text', 'file_name', 'line_number'],
)
...
def detect_mock_error(build_log, root_log):
    ...
    return analyze_log_file(
        build_log_detectors, build_log
    ) or analyze_log_file(root_log_detectors, root_log)
